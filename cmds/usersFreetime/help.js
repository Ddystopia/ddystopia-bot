...
module.exports.run = async (client, message, args) => {
  ...
  const embedProfile = new EmbedInstance('Команды профиля').addField(
    '\u200B',
    `
	1. >profile [пинг юзера] - показать профиль, если не будет аргумента, или он будет некоректен, покажет ваш
	2. >birthday [DD-MM-YYYY] - устоновить день рождение
	3. >about [text] - рассказать о себе, чтоб пеевести строку, используйте \\n
	4. >marry [пинг юзера] - пожениться с пользователем, если пользователь напишет такую же комнду с пингом на вас, в профиле отобразит вашу половинку 
	5. >tear - порвать брак
	6. >ship [пинг юзера]{2,} - шипперство, от 2х и более пингов
...
3. >wheel [sum / all]
4. >cf [sum / all] [t / h]
5. >money [пинг юзера] - узнать баланс (если вы допустите ошибку в аргументах или не укажите их, покажет ваши значения)
6. >give [sum] [пинг юзера] - Больше 10к нельзя(если вы будете передавать много раз, то на 50к вам дадут пред, на 60к мут)
...
Если есть возможность, предлогайте свои графики, а то это 2.35 просто затычка
  ...
  const embedShop = new EmbedInstance('Команды магазина').addField(
    '\u200B',
    `
...
2. >reward [sum / -all(снять всё)] [пинг юзера]
3. >setRoleEveryone [ИМЯ роли] [basic / -] [delIfAnother] - без флага basic даст все онлайн пользователям роль, с флагом basic и без delIfAnother даст только тем, у кого ролей нет, с delIfAnother ещё и удалит у тех, у кого есть роли
4. >shop addRole [пинг роли] [sum] - Добавить роль в таблицу / Изменить цену роли
5. >shop removeRole [пинг роли] - Удалить роль из таблицы
6. >embed [пинг канала] [JSON] - для генерации json-а используйте https://eb.nadeko.bot/ , сами не пишите
7. >cities clear - очистить массив слов
8. >cities addtWords [JSON] - добавить массив слов
9. >cities setWords [JSON] - добавить массив слов
		`
  )
  try {
