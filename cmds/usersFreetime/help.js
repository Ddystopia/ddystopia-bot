...
module.exports.run = async (message, args) => {
  ...
  const embedProfile = new EmbedInstance('Команды профиля')
	...
	1. >profile [@упоминание] - показать профиль, если не будет аргумента, или он будет некоректен, покажет ваш. Псевдонимы - юзер, профиль
	2. >rank [@упоминание] - узнать короткие показатели своего уровня. Псевдонимы - ранг 
	3. >birthday [YYYY-MM-DD] - установить день рождения
	4. >about [text] - рассказать о себе, чтоб пеевести строку, используйте \\n
	5. >marry [@упоминание] - пожениться с пользователем, вы оба должны иметь лут💍, если пользователь напишет такую же комнду с пингом на вас, в профиле отобразит вашу половинку 
	6. >tear - порвать брак
	7. >loot  - собрать случайный лут
	8. >giveLoot [@упоминание] [ [лут]+ / all ] - дать кому-то лут
	9. >lootBox - открыть лутбокс, который вы должны предварительно купить (🎁)
	10. >ship [@упоминание]{2,} - шипперство, от 2х и более пингов
  ...
  const embedGames = new EmbedInstance('Команды игр').addField(
    '\u200B',
    `
		1. >ttt [bet] [@упоминание] - крестики нолики, бот поставит реакцию, чтоб присоединиться, надо на неё нажать, играть просто отправляя числа, если указать пользователя, другие будут игнорироваться, аргумента можно менять местами Псевдонимы - ттт
		2. >cities symbol - узнать букву, с которой должно начинаться слово, полезно когда кто-то удалил или изменил своё сообщение
	`
  )
...
1. >daily [up / level] [n] - раз в 12 часов, можно улучшать. Псевдонимы - timely
2. >br [sum / all]
3. >wheel [sum / all]
4. >cf [sum / all] [t / h]
5. >money [@упоминание] - узнать баланс (если вы допустите ошибку в аргументах или не укажите их, покажет ваши значения). Псевдонимы - $, balance
...
3. >market sell [ [лут]+/all ] - продать лут
Пример: >market buy 🎁
Пример: >market buy 🎁🎟
Пример: >market sell all
`
  )
...
13. >levelingRoles add [[Имя роли]] [уровень] - добавить роль
14. >levelingRoles remove [[Имя роли]] - убрать роль
Пример: >levelingRoles add [Нолайфер] 34
		`
  )
  const embeds = [
    embedIntroduction,
