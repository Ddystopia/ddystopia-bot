...
module.exports.run = async (client, message, args) => {
  ...
  const embedProfile = new EmbedInstance('Команды профиля')
	...
	7. >giveLoot [@упоминание] [ [лут]+ / all ] - дать кому-то лут
  ...
  const embedGames = new EmbedInstance('Команды игр').addField(
    '\u200B',
    `
		1. >ttt [bet] [@упоминание] - крестики нолики, бот поставит реакцию, чтоб присоединиться, надо на неё нажать, играть просто отправляя числа, если указать пользователя, другие будут игнорироваться, аргумента можно менять местами
