...
class Game {
	constructor(number, client) {
		this.roles = {};
		this.roles.mafias =
			number >= 10 ? Math.floor(number / 3) - 1 : Math.floor(number / 3);
	...
	async night() {
		...
		let toKill = Object.entries(this.votes)[0] ? +Object.entries(this.votes).sort((a, b) => b[1] - a[1])[0][0] : null;
		tmpCounter = 0;
		this.votedPlayers = new Set();
		this.votes = {};
		this.vote = false;
		...
		if (this.players.get("dons")) {
			...
			toDonCheck.forEach(item => item[0] ? item[0].send(this.alivePlayers[item[1] + 1].role) : null)
			Object.entries(this.votes).forEach(item => item[0] ? item[0].send('Vote is end') : null);
			tmpCounter = 0;
			this.votedPlayers = new Set();
			this.votes = {};
	...
	async day(winner) {
		...
		let toKill = Object.entries(this.votes)[0] ? +Object.entries(this.votes).sort((a, b) => b[1] - a[1])[0][0] + 1 : null;
		this.votedPlayers = new Set();
		this.votes = {};
		this.vote = false;
		const username = this.alivePlayers[toKill].member.nickname || this.alivePlayers[toKill].member.user.username;
	...
	async win(winner) {
		...
		this.players.get(winner).forEach(item => {
			...
			profile.coins += 5000;
			fs.writeFile(__dirname.replace(/cmds$/, '') + `profiles/${item.user.id}.json`, JSON.stringify(profile), err => err ? console.log(err) : null);
		})
		game = null;
	}
...
class Commands {
	...
	static vote(message, args) {
		...
		if (message.channel.type == 'dm')
			game.votes[message.author] = +args[0];
		else {
