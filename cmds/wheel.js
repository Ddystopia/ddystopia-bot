...
module.exports.run = async (bot, message, args) => {
	...
	const rand = userGames < 15 ? randomInteger(0, 7) : randomInteger(0, 99) <= parcent ? randomInteger(4, 7) : randomInteger(0, 3);
	...
	exampleEmbed
		.addField('Расчёт', `Вы выиграли ${Math.floor(bet * factor)} монет\nНа вашем счету теперь ${profile.coins} монет\n`);
