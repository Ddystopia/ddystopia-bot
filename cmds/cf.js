...
module.exports.run = async (client, message, args) => {
	...
	const side = userGames < 11 ? sides[randomInteger(0, 1)] : randomInteger(0, 99) <= parcent ? betedSide : loseSide;
	...
	if (betedSide == side) {
		...
		exampleEmbed
			...
			.addField('Расчёт', `Вы выиграли ${bet} монет\nНа вашем счету теперь ${profile.coins} монет\n`);
	} else {
		profile.coins -= bet;
		exampleEmbed
			.setImage(sidesImages[side])
			.addField('Расчёт', `Вы потеряли ${bet} монет\nНа вашем счету теперь ${profile.coins} монет\n`);
	}
