...
class RolesBoard {
	static shopList(message) {
		const shopList = new Discord.MessageEmbed()
			...
			.setTimestamp();
		let rolesClone = {}
		Object.entries(roles).sort((a, b) => +b[1] - +a[1]).forEach(e => {
		...
		fs.writeFile(__dirname.replace(/cmds$/, '') + `roles.json`, JSON.stringify(roles), err => err ? console.log(err) : null);
	...
	static removeRole(message, args) {
		...
		fs.writeFile(__dirname.replace(/cmds$/, '') + `roles.json`, JSON.stringify(roles), err => err ? console.log(err) : null);
		let rolesClone = {}
		Object.entries(roles).sort((a, b) => +b[1] - +a[1]).forEach(e => rolesClone[e[0]] = e[1])
		roles = rolesClone;
		rolesClone = null;
		fs.writeFile(__dirname.replace(/cmds$/, '') + `roles.json`, JSON.stringify(roles), err => err ? console.log(err) : null);
