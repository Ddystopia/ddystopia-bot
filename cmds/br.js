...
module.exports.run = async (client, message, args) => {
	...
	const realNum = userGames < 15 ? randomInteger(0, 100) : randomInteger(0, 99) <= parcent ? randomInteger(65, 99) : randomInteger(0, 64);
	let jackpot = '';

	if (profile.coins < bet || bet <= 0) return message.reply('Не хватает монет');

	if (betNum == realNum && randomInteger(0, 1) && games.get(message.author.id) < 20) jackpot = ', но вам улыбнулясь удача, поэтому вы получаете надбавку в 10000 и множительна выйграш 100!!!';
	...
	if (realNum >= 65) {
		...
		exampleEmbed
			.addField('Расчёт', `Вам выпало число ${realNum}, и вы выиграли ${bet * factor} монет${jackpot}\nНа вашем счету теперь ${profile.coins} монет\n`);
	} else {
		profile.coins -= bet;
		if (jackpot) profile.coins += 10000;
		exampleEmbed
			.addField('Расчёт', `Вам выпало число ${realNum}, и вы потеряли ${bet} монет${jackpot}\nНа вашем счету теперь ${profile.coins} монет\n`);
	}
