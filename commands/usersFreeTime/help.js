...
module.exports.run = async (message, [page]) => {
  ...
  if (!guildDB.bancrotRole)
    embedBank.setDescription(
      'Этот блок комманд пока не доступнен для этого сервера, попросите администратора добавить банкротскую роль, по задумке должна быть отврадительного цвета с высоким приоритетом и небольшими ограничениями'
    )
...
Доступные действия(их писать вместо [conmmand])
 ...
 | fact | 
  ...
  const embedSettings = new EmbedInstance('Настройки', guildDB.prefix, [
    ...
    '|pref|bancrotRole [set/clear] [id/упоминание] - установить/убрать роль для тех, кто не выплатил кредит, без неё не работает банк',
    '|pref|greetingChannel [set/clear] [id/упоминание] - установить/убрать канал для приветствия новых учасников',
    '|pref|blacklist [add/remove] [id/упоминание] [id/упоминание]... - добавить/удалить людей(человека), которых будет игнорировать бот',
    '|pref|imageChannels [add/remove] [id/упоминание] [id/упоминание]... - добавить/удалить кланал(ы), в которых будут выдаваться деньши за картинки(например канал с мемами)',
    '|pref|noCommandsChannels [add/remove] [id/упоминание] [id/упоминание]... - добавить/удалить кланал(ы), в котором(ых) бот будет игнорировать *комманды*',
    '|pref|wordsGameChannels [add/remove][id/упоминание] [id/упоминание]... - добавить/удалить кланал(ы), в которых будет проводится игра в слова',
  ])
  const embeds = [
    embedProfile,
    embedGames,
